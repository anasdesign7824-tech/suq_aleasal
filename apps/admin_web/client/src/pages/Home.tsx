...
import { adminProductTypeLabel, adminStatusMeta, type AdminStatusContext } from "@/lib/ui-consistency";
import { BannersPanel, LogisticsPanel, TaxonomyPanel } from "@/components/AdminOperations";
...
import { AdminStoreVerificationPanel } from "@/components/AdminStoreVerification";
import { AdminPlansPanel } from "@/components/AdminPlansPanel";
import { AdminSafeImage } from "@/components/AdminVisuals";

type ViewKey = "overview" | "products" | "stores" | "requests" | "merchant-applications" | "store-verification" | "plans" | "banners" | "taxonomy" | "logistics" | "admins" | "audit" | "users" | "notifications" | "analytics";
type ProductRow = { id: string; store_id: string; taxonomy_id?: string | null; name_ar: string; name_en?: string | null; description?: string | null; product_type: string; grade_level?: number | null; status: string; is_featured: boolean; metadata?: Record<string, unknown> | null; product_images?: Array<{ id?: string; image_url: string; sort_order?: number | null }> | null };
...
const navItems: Array<{
  ...
  { key: "overview", label: "نظرة عامة", icon: LayoutDashboard, group: "التشغيل" },
  { key: "products", label: "الكتالوج", icon: Box, group: "التشغيل" },
  { key: "stores", label: "المتاجر", icon: Store, group: "التشغيل" },
  { key: "requests", label: "طلبات التواصل", icon: ClipboardList, group: "التشغيل" },
  { key: "merchant-applications", label: "طلبات التجار", icon: Handshake, group: "دورة التاجر" },
  { key: "store-verification", label: "توثيق المتاجر Pro", icon: ShieldCheck, group: "دورة التاجر" },
  { key: "plans", label: "الخطط والحملة والدفع", icon: WalletCards, group: "دورة التاجر" },
  { key: "banners", label: "البنرات", icon: ImagePlus, group: "المحتوى والمرجع" },
  { key: "taxonomy", label: "التصنيفات", icon: Layers3, group: "المحتوى والمرجع" },
  { key: "logistics", label: "التوصيل والاستلام", icon: Truck, group: "المحتوى والمرجع" },
  { key: "admins", label: "المديرون", icon: UserCog, group: "الحوكمة" },
  { key: "audit", label: "سجل التدقيق", icon: History, group: "الحوكمة" },
  { key: "users", label: "المستخدمون", icon: Users, group: "الحسابات والتحليل" },
  { key: "notifications", label: "الإشعارات", icon: BellRing, group: "الحسابات والتحليل" },
  { key: "analytics", label: "الإحصاءات", icon: BarChart3, group: "الحسابات والتحليل" },
];

export const ADMIN_NAVIGATION_ITEM_COUNT = navItems.length;
...
function statusBadge(status: string, context: AdminStatusContext) {
  const style = adminStatusMeta(status, context);
...
function ProductThumb({ product }: { product: ProductRow }) {
  ...
  if (!imageUrl || broken) {
    return <div className="grid size-11 shrink-0 place-items-center rounded-xl bg-[#fff0d6]" aria-label="لا توجد صورة للمنتج"><Hexagon className="size-4 text-[#9c5a00]" /></div>;
  }
  return <img src={imageUrl} alt="" className="size-11 shrink-0 rounded-xl object-cover" onError={() => setBroken(true)} />;
}
...
function StoresPanel({ state, onRefresh, onModerate, onDelete, onSelect }: { state: LoadState<StoreRow[]>; onRefresh: () => void; onModerate: (id: string, action: "approve" | "reject" | "suspend" | "reactivate") => Promise<void>; onDelete: (id: string) => Promise<void>; onSelect?: (store: StoreRow) => void }) {
  return <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{state.loading ? <div className="admin-card md:col-span-2 xl:col-span-3"><LoadingState /></div> : state.error ? <div className="admin-card md:col-span-2 xl:col-span-3"><ErrorState message={state.error} onRefresh={onRefresh} /></div> : (state.data ?? []).length === 0 ? <div className="admin-card md:col-span-2 xl:col-span-3"><EmptyState title="لا توجد متاجر في Production" description="قاعدة البيانات الحالية لا تحتوي متاجر. ستظهر المتاجر هنا بعد إنشاء طلب حقيقي واعتماده." onRefresh={onRefresh} /></div> : (state.data ?? []).map((store) => <article className="admin-card relative overflow-hidden p-6" key={store.id}><div className="absolute left-0 top-0 h-20 w-20 rounded-br-[42px] bg-[#fff0d6]" /><div className="relative flex items-start justify-between"><StoreThumb store={store} />{statusBadge(store.status, "store")}</div><h3 className="relative mt-8 text-lg font-bold text-[#3f281d]">{store.name_ar}</h3><p className="relative mt-2 min-h-12 text-sm leading-6 text-[#806b5a]">{store.description ?? "لا يوجد وصف للمتجر."}</p><div className="relative mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#f1e6da] pt-4"><span className="text-xs text-[#876f5c]">{store.is_verified ? "متجر معتمد" : "يحتاج مراجعة"}</span><div className="flex flex-wrap items-center justify-end gap-1">{onSelect && <Button data-action="store.details" aria-label={`استعراض تفاصيل المتجر ${store.name_ar}`} onClick={() => onSelect(store)} variant="ghost" size="sm" className="text-[#8b5a2b]">استعراض التفاصيل<ChevronLeft className="mr-1 size-4" /></Button>}{store.status === "pending" && <><Button onClick={() => void onModerate(store.id, "approve")} variant="outline" size="sm" className="border-emerald-200 text-emerald-800">اعتماد</Button><Button onClick={() => void onModerate(store.id, "reject")} variant="outline" size="sm" className="border-red-200 text-red-800">رفض</Button></>}{store.status === "active" && <Button onClick={() => void onModerate(store.id, "suspend")} variant="outline" size="sm" className="border-red-200 text-red-800">تعليق</Button>}{["rejected", "suspended", "paused"].includes(store.status) && <Button onClick={() => void onModerate(store.id, "reactivate")} variant="outline" size="sm" className="border-emerald-200 text-emerald-800">إعادة تفعيل</Button>}<Button data-action="store.delete" aria-label={`حذف المتجر ${store.name_ar}`} onClick={() => void onDelete(store.id)} variant="outline" size="sm" className="border-red-200 text-red-800"><Trash2 className="ml-1 size-3.5" />حذف</Button></div></div></article>)}</section>;
}
...
export default function Home({ session, onLogout }: { session: AdminSessionPayload; onLogout: () => Promise<void> }) {
  ...
  const moderateStore = async (id: string, action: "approve" | "reject" | "suspend" | "reactivate") => { const confirmation = action === "approve" ? "اعتماد المتجر سيحوّل الحساب إلى تاجر ويفعّل المنتجات المعلقة. هل تريد المتابعة؟" : action === "reject" ? "رفض المتجر سيمنع ظهوره للمستخدمين. هل تريد المتابعة؟" : action === "suspend" ? "تعليق المتجر سيخفيه عن المستخدمين مؤقتًا. هل تريد المتابعة؟" : "إعادة تفعيل المتجر ستعيد ظهوره وتزامن حالته. هل تريد المتابعة؟"; if (!window.confirm(confirmation)) return; try { await adminApi.moderateStore(id, action); toast.success(action === "approve" ? "تم تفعيل المتجر ومزامنة حساب التاجر والمنتجات والإشعار." : action === "reactivate" ? "تمت إعادة تفعيل المتجر ومزامنة حالته." : action === "suspend" ? "تم تعليق المتجر ومزامنة حالته." : "تم رفض المتجر ومزامنة القرار."); refreshStores(); refreshProducts(); refreshOverview(); } catch (error) { toast.error(error instanceof Error ? error.message : "تعذر تحديث المتجر."); } };
  ...
  const titles: Record<ViewKey, { eyebrow: string; title: string; subtitle: string }> = {
    overview: { eyebrow: "لوحة التشغيل", title: "صباح الخير، فريق عسلكم", subtitle: "إليك لقطة مباشرة من مصدر Supabase الحي." },
    ...
    "merchant-applications": { eyebrow: "دورة التاجر", title: "طلبات التجار", subtitle: "طلبات حقيقية من تطبيق العميل، مع قرار مراجعة محفوظ في بيئة الإنتاج وسجل التدقيق." },
    "store-verification": { eyebrow: "الخدمة الاحترافية (Pro)", title: "توثيق المتاجر", subtitle: "مراجعة مستقلة للرسوم والمستندات والشارة، ولا تعني فتح المتجر تلقائيًا." },
    plans: { eyebrow: "الاشتراكات والدفع", title: "الخطط وحملة الافتتاح", subtitle: "الأسعار والخصم والحوالة المحلية وتفعيل المزايا من المصدر الحي." },
    ...
    logistics: { eyebrow: "تشغيل المتجر", title: "التوصيل ونقاط الاستلام", subtitle: "إدارة الطرق والنطاقات ونقاط الاستلام من الجداول التي يقرأها تطبيق العميل." },
  ...
  const navigation = <nav data-testid="admin-navigation" data-nav-item-count={ADMIN_NAVIGATION_ITEM_COUNT} aria-label="التنقل الإداري" className="mt-8 min-h-0 flex-1 space-y-1.5 overflow-y-auto overscroll-contain pr-1">{navItems.map((item, index) => { const Icon = item.icon; const isFirstInGroup = index === 0 || navItems[index - 1].group !== item.group; return <div key={item.key}>{isFirstInGroup && <p className="mb-2 mt-4 px-3 text-[10px] font-bold tracking-[.16em] text-[#cba983] first:mt-0">{item.group}</p>}<button onClick={() => { setActiveView(item.key); setSidebarOpen(false); }} className={`flex w-full items-center gap-3 rounded-2xl px-3.5 py-3 text-right text-sm font-semibold transition-all ${activeView === item.key ? "bg-[#f39c12] text-[#4f2e1f] shadow-[0_10px_25px_rgba(243,156,18,.18)]" : "text-[#ead8c6] hover:bg-white/10 hover:text-white"}`}><Icon className="size-[18px]" />{item.label}{item.key === "requests" && (requests.data ?? []).length > 0 && <span className="mr-auto grid size-5 place-items-center rounded-full bg-white/15 text-[10px]">{requests.data?.length}</span>}</button></div>; })}</nav>;
  const counts = overview.data?.counts ?? {};

  return <main className="min-h-screen bg-[#fbf8f2]" dir="rtl"><aside className="fixed inset-y-0 right-0 z-30 hidden w-72 flex-col bg-[#4f2e1f] px-5 py-7 shadow-[-18px_0_45px_rgba(79,46,31,.10)] lg:flex"><AssalkomLogo /><div className="mt-6 flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.05] p-3"><div className="grid size-10 place-items-center rounded-xl bg-[#f9e7c7] text-[#9c5a00]"><ShieldCheck className="size-5" /></div><div><p className="text-xs font-bold text-white">دار إدارة العسل</p><p className="mt-0.5 text-[10px] text-[#d6b994]">مركز القرار والتدقيق</p></div></div>{navigation}<div className="mt-auto rounded-3xl border border-white/10 bg-white/[.06] p-4"><div className="flex items-center gap-2 text-[#ffe4b0]"><ShieldCheck className="size-4" /><span className="text-xs font-bold">مصدر Supabase الحي</span></div><p className="mt-2 text-xs leading-6 text-[#ebdaca]">المصدر الحي متصل عبر الخادم المحلي المصرح.</p></div></aside><div className="min-h-screen lg:mr-72"><header className="sticky top-0 z-20 flex items-center justify-between border-b border-[#eee1d0] bg-[#fbf8f2]/90 px-5 py-4 backdrop-blur-xl lg:px-8"><div className="flex items-center gap-3"><Button onClick={() => setSidebarOpen(true)} variant="ghost" size="icon" className="lg:hidden"><Menu className="size-5" /></Button><div><p className="text-[11px] font-bold tracking-[.18em] text-[#a37747]">{heading.eyebrow}</p><h2 className="mt-0.5 text-base font-bold text-[#3f281d]">{heading.title}</h2></div></div><div className="flex items-center gap-2"><Badge variant="outline" className="hidden border-dashed border-[#b7dfc2] bg-[#effaf1] text-[#26733d] sm:inline-flex">الإنتاج</Badge><Button onClick={() => void onLogout()} variant="ghost" size="sm" className="text-[#6f5b4c]">خروج</Button><Button onClick={() => setActiveView("notifications")} variant="ghost" size="icon" className="relative"><Bell className="size-5 text-[#6f5b4c]" /></Button><div className="grid size-9 place-items-center rounded-2xl bg-[#f2e8dd] text-sm font-bold text-[#704424]">{(session.user.email ?? "ع").slice(0, 1).toUpperCase()}</div></div></header><div className="px-5 py-7 lg:px-8 lg:py-8"><div className="mb-6 max-w-2xl"><div className="section-kicker">{heading.eyebrow}</div><p className="mt-2 text-sm leading-7 text-[#806b5a]">{heading.subtitle}</p></div>{activeView === "overview" && <><section className="relative overflow-hidden rounded-[28px] border border-[#eadcc9] bg-[#5b3623] px-6 py-6 text-white shadow-[0_20px_55px_rgba(79,46,31,.16)] lg:px-8"><div aria-hidden="true" className="absolute inset-y-0 left-0 h-full w-1/2 bg-[radial-gradient(circle_at_30%_30%,rgba(243,156,18,.45),transparent_32%),linear-gradient(135deg,rgba(243,156,18,.25),transparent)] opacity-70" /><div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_260px] lg:items-end"><div className="max-w-2xl"><div className="flex items-center gap-2 text-[#ffe4b0]"><Sparkles className="size-4" /><span className="text-xs font-semibold">موجز التشغيل اليومي</span></div><h1 className="mt-3 text-3xl font-bold leading-tight lg:text-[34px]">قرار واحد يحتاج متابعتك اليوم.</h1><p className="mt-2 max-w-xl text-sm leading-7 text-[#f8e6cb]">البيانات المعروضة هنا تأتي من مصدر Supabase الحي عبر الخادم المحلي، ولا تستخدم بيانات تجريبية (Demo).</p><div className="mt-5 flex flex-wrap gap-3"><Button onClick={refreshOverview} className="bg-[#f39c12] text-[#4f2e1f] shadow-none hover:bg-[#ffb340]"><RefreshCw className="ml-2 size-4" />تحديث المصدر</Button><Button onClick={() => overview.data && downloadSnapshot(overview.data)} disabled={overview.loading || !overview.data || !!overview.error} variant="outline" className="border-[#d7a768] bg-transparent text-white hover:bg-white/10 hover:text-white"><Download className="ml-2 size-4" />تصدير الموجز</Button></div></div><div className="rounded-2xl border border-white/10 bg-white/[.08] p-4 backdrop-blur-sm"><p className="text-[10px] font-bold tracking-[.15em] text-[#ffe4b0]">صحة مساحة العمل</p><div className="mt-3 space-y-3"><div className="flex items-center justify-between text-xs"><span className="text-[#f5e3cb]">مصدر البيانات</span><span className="font-bold text-[#ffe4b0]">الإنتاج</span></div><div className="flex items-center justify-between text-xs"><span className="text-[#f5e3cb]">المدير</span><span className="max-w-32 truncate font-bold text-white" dir="ltr">{session.user.email}</span></div><div className="flex items-center justify-between text-xs"><span className="text-[#f5e3cb]">الدور</span><span className="font-bold text-[#f2c777]">{session.role.nameAr}</span></div></div></div></div></section>{overview.error ? <section className="mt-4 admin-card"><ErrorState message={overview.error} onRefresh={refreshOverview} /></section> : <><section className="mt-4 flex flex-wrap items-center gap-2 rounded-2xl border border-[#b7dfc2] bg-[#effaf1] px-4 py-3 text-xs text-[#26733d]"><span className="font-bold">الحالة التشغيلية:</span><Badge variant="outline" className="border-[#8bca9b] bg-white/60 text-[#26733d]">مصدر Supabase الحي</Badge><span className="h-1 w-1 rounded-full bg-[#8bca9b]" /><span>لا توجد بيانات تجريبية (Demo) في هذا المسار</span></section><section className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="المستخدمون" value={overview.loading ? "—" : counts.users ?? 0} note="من جدول الهوية التشغيلية" icon={PackageCheck} tone="gold" /><MetricCard label="المنتجات" value={overview.loading ? "—" : counts.products ?? 0} note="من المصدر الحقيقي" icon={Box} tone="brown" /><MetricCard label="المتاجر" value={overview.loading ? "—" : counts.stores ?? 0} note="بعد الاعتماد والنشر" icon={Store} tone="cream" /><MetricCard label="المناطق المرجعية" value={overview.loading ? "—" : counts.regions ?? 0} note="للتصفية الجغرافية" icon={Hexagon} tone="olive" /></section></>}<section className="mt-6 grid gap-6 xl:grid-cols-[minmax(330px,.8fr)_minmax(0,1.45fr)]"><RequestsPanel state={requests} onSelect={setSelectedRequest} onRefresh={refreshRequests} /><ProductTable state={products} onRefresh={refreshProducts} onDelete={deleteProduct} onSelect={setSelectedProduct} compact /></section></>}{activeView === "products" && <div className="space-y-6"><ProductCreationPanel stores={stores.data ?? []} onCreated={() => { refreshProducts(); refreshOverview(); }} /><ProductTable state={products} onRefresh={refreshProducts} onDelete={deleteProduct} onSelect={setSelectedProduct} /></div>}
  {activeView === "stores" && <StoresPanel state={stores} onRefresh={refreshStores} onModerate={moderateStore} onDelete={deleteStore} onSelect={setSelectedStore} />}{activeView === "merchant-applications" && <MerchantApplicationsPanel state={merchantApplications} onRefresh={refreshMerchantApplications} onReviewed={refreshMerchantApplications} />}  {activeView === "store-verification" && <AdminStoreVerificationPanel />}{activeView === "plans" && <AdminPlansPanel />}{activeView === "banners" && <BannersPanel />}{activeView === "taxonomy" && <TaxonomyPanel />}{activeView === "logistics" && <LogisticsPanel stores={stores.data ?? []} />}{activeView === "admins" && <div className="space-y-6"><AdminCreationPanel /><AdminUsersPanel /></div>}{activeView === "audit" && <AuditPanel />}{activeView === "users" && <UsersPanel />}{activeView === "notifications" && <NotificationsPanel />}{activeView === "analytics" && <AnalyticsPanel />}{activeView === "requests" && <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]"><RequestsPanel state={requests} onSelect={setSelectedRequest} onRefresh={refreshRequests} /><aside className="admin-card p-6"><Filter className="size-5 text-[#9c5a00]" /><h3 className="mt-4 text-lg font-bold text-[#3f281d]">حالة المصدر</h3><p className="mt-2 text-sm leading-7 text-[#806b5a]">هذه لوحة خاصة محلية. أي إجراء إداري يمر عبر الخادم المحلي ثم يتحقق من الصلاحية قبل الكتابة في قاعدة البيانات الحية.</p><Button onClick={refreshRequests} variant="outline" className="mt-5 w-full border-[#e3c28d] text-[#8b5a2b]"><RefreshCw className="ml-2 size-4" />تحديث الطلبات</Button></aside></section>}</div></div><Sheet open={sidebarOpen} onOpenChange={setSidebarOpen}><SheetContent side="right" className="flex h-full w-72 flex-col overflow-hidden border-0 bg-[#4f2e1f] p-5 text-white"><SheetHeader className="sr-only"><SheetTitle>التنقل</SheetTitle></SheetHeader><div className="flex items-center justify-between"><AssalkomLogo /><Button onClick={() => setSidebarOpen(false)} variant="ghost" size="icon" className="text-white hover:bg-white/10 hover:text-white"><X className="size-5" /></Button></div>{navigation}</SheetContent></Sheet><Sheet open={selectedRequest !== null} onOpenChange={(open) => !open && setSelectedRequest(null)}><SheetContent side="left" className="w-full overflow-y-auto sm:max-w-md"><SheetHeader><SheetTitle className="text-right text-xl text-[#3f281d]">تفاصيل طلب التواصل</SheetTitle></SheetHeader>{selectedRequest && <div className="mt-7 space-y-5 text-right"><div className="rounded-3xl bg-[#fff7e9] p-5"><div className="flex items-center justify-between"><span className="text-xs text-[#8d725a]">الحالة</span>{statusBadge(selectedRequest.status, "request")}</div><h3 className="mt-4 text-lg font-bold text-[#3f281d]">{selectedRequest.subject}</h3><p className="mt-3 text-sm leading-7 text-[#6f5b4c]">{selectedRequest.body ?? "بدون وصف"}</p></div><form onSubmit={answerSelectedRequest} className="rounded-2xl border border-[#dfc6a9] p-4"><p className="text-sm font-semibold text-[#4f2e1f]">إرسال رد حقيقي</p><Input required value={replyBody} onChange={(event) => setReplyBody(event.target.value)} className="mt-3 rounded-xl border-[#eadcc9]" placeholder="اكتب الرد هنا" /><Button type="submit" disabled={replying || !replyBody.trim()} className="mt-3 w-full bg-[#4f2e1f] hover:bg-[#6b412a]">{replying ? "جارٍ إرسال الرد…" : "إرسال الرد"}</Button></form></div>}</SheetContent></Sheet><Sheet open={selectedProduct !== null} onOpenChange={(open) => !open && setSelectedProduct(null)}><SheetContent side="left" className="w-full overflow-y-auto sm:max-w-2xl"><SheetHeader><SheetTitle className="sr-only">تعديل المنتج</SheetTitle></SheetHeader>{selectedProduct && <ProductEditPanel product={selectedProduct} onClose={() => setSelectedProduct(null)} onSaved={() => { setSelectedProduct(null); refreshProducts(); refreshOverview(); }} />}</SheetContent></Sheet><Sheet open={selectedStore !== null} onOpenChange={(open) => !open && setSelectedStore(null)}><SheetContent side="left" className="w-full overflow-y-auto sm:max-w-lg"><SheetHeader><SheetTitle className="text-right text-xl text-[#3f281d]">تفاصيل المتجر</SheetTitle></SheetHeader>{selectedStore && <div className="mt-7 space-y-5 text-right"><div className="overflow-hidden rounded-3xl border border-[#eadcc9] bg-[#fff7e9]"><div className="relative h-36 w-full bg-gradient-to-l from-[#4f2e1f] to-[#9c5a00]"><AdminSafeImage src={selectedStore.cover_url} alt={`غلاف متجر ${selectedStore.name_ar}`} /><div className="absolute bottom-3 right-4 size-16 overflow-hidden rounded-2xl border-2 border-white bg-[#fff0d6] shadow-lg"><AdminSafeImage src={selectedStore.logo_url} alt={`شعار متجر ${selectedStore.name_ar}`} /></div></div><div className="p-5"><div className="flex items-center justify-between gap-3"><h3 className="text-xl font-bold text-[#3f281d]">{selectedStore.name_ar}</h3>{statusBadge(selectedStore.status, "store")}</div><p className="mt-3 text-sm leading-7 text-[#6f5b4c]">{selectedStore.description ?? "لا يوجد وصف مسجل."}</p></div></div><div className="grid gap-3 rounded-2xl border border-[#eadcc9] p-4 text-sm text-[#6f5b4c] sm:grid-cols-2"><p><span className="font-semibold text-[#4f2e1f]">الحالة:</span> {selectedStore.status}</p><p><span className="font-semibold text-[#4f2e1f]">الاعتماد:</span> {selectedStore.is_verified ? "معتمد" : "غير معتمد"}</p><p><span className="font-semibold text-[#4f2e1f]">معرّف المتجر:</span> <span dir="ltr">{selectedStore.id}</span></p><p><span className="font-semibold text-[#4f2e1f]">معرّف التاجر:</span> <span dir="ltr">{selectedStore.merchant_id ?? "غير مسجل"}</span></p><p><span className="font-semibold text-[#4f2e1f]">معرّف المنطقة:</span> <span dir="ltr">{selectedStore.region_id ?? "غير مسجل"}</span></p><p><span className="font-semibold text-[#4f2e1f]">الهاتف:</span> غير معروض في عقد قائمة المتاجر الحالي</p></div></div>}</SheetContent></Sheet></main>;
}
