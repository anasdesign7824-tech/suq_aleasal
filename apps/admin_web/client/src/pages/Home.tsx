...
type ProductRow = { id: string; store_id: string; name_ar: string; product_type: string; status: string; is_featured: boolean; metadata?: Record<string, unknown> };
type StoreRow = { id: string; name_ar: string; description?: string | null; status: string; is_verified: boolean; region_id?: string | null };
...
function AssalkomLogo() {
  return <div className="flex items-center gap-3"><img className="h-12 w-12 rounded-2xl bg-[#fffaf0] object-contain p-1 shadow-[0_8px_22px_rgba(243,156,18,.20)]" src={logoUrl} alt="شعار عسلكم" /><div><p className="text-xl font-bold tracking-tight text-[#fffaf0]">عسلكم</p><p className="mt-0.5 text-[10px] font-medium tracking-[.22em] text-[#f8d59c]">SOUQ AL ASSAL</p></div></div>;
}
...
function ProductTable({ state, onRefresh, onDelete, compact = false }: { state: LoadState<ProductRow[]>; onRefresh: () => void; onDelete?: (id: string) => Promise<void>; compact?: boolean }) {
...
export default function Home({ session, onLogout }: { session: AdminSessionPayload; onLogout: () => Promise<void> }) {
  ...
  const titles: Record<ViewKey, { eyebrow: string; title: string; subtitle: string }> = {
    ...
    users: { eyebrow: "Customer Directory", title: "المستخدمون", subtitle: "قراءة الحسابات والملفات من المصدر الحقيقي دون منح صلاحيات إدارة." },
    notifications: { eyebrow: "Communication", title: "الإشعارات", subtitle: "إشعارات مرتبطة بالمستخدمين الحقيقيين داخل التطبيق." },
    analytics: { eyebrow: "Operational Analytics", title: "الإحصاءات التشغيلية", subtitle: "مؤشرات مشتقة من جداول Production دون بيانات تقديرية." },
  };
