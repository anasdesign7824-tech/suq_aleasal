...
import { Activity, BarChart3, BellRing, Clock3, Globe2, Mail, MapPin, RefreshCw, Send, ShieldCheck, Store, Trash2, Users } from "lucide-react";
...
export function UsersPanel() {
  ...
  const removeUser = async (user: UserRow) => {
    ...
    try { await adminApi.deleteUser(user.id); toast.success("تم حذف المستخدم."); await load(); }
...
export function NotificationsPanel() {
  ...
  return <section className="space-y-6"><div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]"><div className="admin-card overflow-hidden"><div className="flex items-start justify-between border-b border-[#eee1d0] px-5 py-5"><div><div className="section-kicker">Communication</div><h2 className="mt-1 text-xl font-bold text-[#342118]">الإشعارات</h2></div><BellRing className="size-5 text-[#9c5a00]" /></div>{loading ? <div className="p-8 text-center"><RefreshCw className="mx-auto size-5 animate-spin text-[#c77d1a]" /></div> : items.length === 0 ? <div className="p-8 text-center text-sm leading-7 text-[#806b5a]">لا توجد إشعارات حقيقية بعد.</div> : <div className="divide-y divide-[#f1e7da]">{items.map((item) => <div key={item.id} className="flex gap-3 px-5 py-4"><Mail className="mt-1 size-4 text-[#9c5a00]" /><div><p className="font-semibold text-[#432a1e]">{item.title_ar}</p><p className="mt-1 text-sm text-[#806b5a]">{item.body_ar ?? "بدون نص"}</p><p className="mt-1 text-xs text-[#8e7a68]" dir="ltr">{item.user_id}</p></div></div>)}</div>}</div><div className="admin-card p-6"><div className="section-kicker">إرسال داخلي</div><h2 className="mt-1 text-xl font-bold text-[#342118]">إشعار لمستخدم أو للجميع</h2><div className="mt-5 space-y-3"><div className="grid grid-cols-2 gap-2"><Button type="button" variant={form.targetMode === "user" ? "default" : "outline"} onClick={() => setForm({ ...form, targetMode: "user" })}>مستخدم واحد</Button><Button type="button" variant={form.targetMode === "broadcast" ? "default" : "outline"} onClick={() => setForm({ ...form, targetMode: "broadcast" })}>إشعار عام</Button></div>{form.targetMode === "user" && <Input dir="ltr" value={form.userId} onChange={(event) => setForm({ ...form, userId: event.target.value })} placeholder="معرف المستخدم UUID" className="rounded-xl border-[#eadcc9]" />}<Input value={form.titleAr} onChange={(event) => setForm({ ...form, titleAr: event.target.value })} placeholder="عنوان الإشعار" className="rounded-xl border-[#eadcc9]" /><Input value={form.bodyAr} onChange={(event) => setForm({ ...form, bodyAr: event.target.value })} placeholder="النص" className="rounded-xl border-[#eadcc9]" /><label className="block text-sm font-semibold text-[#4f2e1f]">صورة اختيارية<Input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={uploadImage} disabled={uploading} className="mt-2 rounded-xl border-[#eadcc9]" />{form.imageUrl && <p className="mt-1 truncate text-xs text-[#806b5a]" dir="ltr">تم الربط: {form.imageUrl}</p>}</label></div><Button disabled={saving || uploading} onClick={() => void send()} className="mt-5 w-full bg-[#4f2e1f] hover:bg-[#6b412a]"><Send className="ml-2 size-4" />{saving ? "جارٍ الإرسال…" : form.targetMode === "broadcast" ? "إرسال للجميع" : "إنشاء الإشعار"}</Button></div></div></section>;
}
