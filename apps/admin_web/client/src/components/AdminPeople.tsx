...
type UserRow = {
  ...
  profile?: { display_name?: string; phone?: string | null; avatar_url?: string | null; bio?: string | null; role?: string; is_active?: boolean } | null;
  ...
  adminMembership?: { user_id?: string; role_id?: string; is_active?: boolean; role?: { code?: string; name_ar?: string } | null } | null;
...
export function UsersPanel() {
  ...
  const load = async (requestedPage = page, requestedSearch = appliedSearch) => { setLoading(true); setError(null); try { const result = await adminApi.users({ page: requestedPage, pageSize, search: requestedSearch || undefined }); setItems(result.items as UserRow[]); setPage(result.page); setTotal(result.total); } catch (requestError) { setError(requestError instanceof Error ? requestError.message : "تعذر قراءة المستخدمين."); } finally { setLoading(false); } };
  const submitSearch = (event: React.FormEvent) => { event.preventDefault(); const nextSearch = search.trim(); setAppliedSearch(nextSearch); void load(1, nextSearch); };
  const removeUser = async (user: UserRow) => {
    ...
    try { await adminApi.deleteUser(user.id); toast.success("تم حذف المستخدم."); await load(items.length === 1 && page > 1 ? page - 1 : page, appliedSearch); }
  ...
  return <section className="admin-card overflow-hidden"><div className="flex items-start justify-between border-b border-[#eee1d0] px-5 py-5"><div><div className="section-kicker">دليل الحسابات</div><h2 className="mt-1 text-xl font-bold text-[#342118]">المستخدمون والتدقيق التشغيلي</h2><p className="mt-2 max-w-3xl text-sm leading-7 text-[#806b5a]">عرض مباشر من users وprofiles وSupabase Auth وطلبات التجار والمتاجر. لا تُعرض معلومات غير مسجلة، وعنوان IP يبقى غير متاح لأن مخطط Production الحالي لا يجمعه.</p></div><div className="flex flex-wrap items-center gap-2"><form onSubmit={submitSearch} className="flex items-center gap-2"><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="ابحث بالاسم أو البريد أو الهاتف" className="h-10 w-64 rounded-xl border-[#eadcc9]" /><Button disabled={loading} type="submit" variant="outline" className="h-10 border-[#eadcc9] text-[#8b5a2b]">بحث</Button></form><Button disabled={loading} onClick={() => void load(page, appliedSearch)} variant="ghost" size="icon" className="text-[#8b5a2b]"><RefreshCw className="size-4" /></Button></div></div>{loading ? <div className="p-8 text-center"><RefreshCw className="mx-auto size-5 animate-spin text-[#c77d1a]" /></div> : error ? <div className="flex flex-wrap items-center justify-between gap-3 p-6 text-sm text-red-800"><span>{error}</span><Button onClick={() => void load(page, appliedSearch)} variant="outline" className="border-red-200 text-red-800"><RefreshCw className="ml-2 size-4" />إعادة المحاولة</Button></div> : items.length === 0 ? <div className="p-8 text-center text-sm leading-7 text-[#806b5a]">لا يوجد مستخدمون في المصدر الحقيقي بعد.</div> : <div className="divide-y divide-[#f1e7da]">{items.map((user) => { const application = user.merchantApplication; const store = user.store; return <article key={user.id} className="p-5"><div className="flex flex-wrap items-start gap-4"><div className="size-14 shrink-0 overflow-hidden rounded-2xl bg-[#f3e9df] text-[#4f2e1f]"><AdminSafeImage src={user.profile?.avatar_url} alt={user.profile?.display_name ?? 'مستخدم عسلكم'} fallbackClassName="grid size-full place-items-center bg-[#f3e9df] text-[#4f2e1f]" /></div><div className="min-w-56 flex-1"><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold text-[#432a1e]">{user.profile?.display_name ?? "مستخدم عسلكم"}</h3><Badge variant="outline" className="border-[#e3c28d] text-[#8b5a2b]">{roleLabel(user.profile?.role)}</Badge><Badge variant="outline" className={user.profile?.is_active === false ? "border-red-200 bg-red-50 text-red-800" : "border-emerald-200 bg-emerald-50 text-emerald-800"}>{user.profile?.is_active === false ? "معطّل" : "نشط"}</Badge></div><p className="mt-1 text-xs text-[#806b5a]" dir="ltr">{user.email ?? "بريد غير متاح"}</p><p className="mt-1 text-[11px] text-[#a18d7a]" dir="ltr">ID: {user.id}</p></div><div className="rounded-2xl bg-[#fff7e9] px-3 py-2 text-right"><p className="text-[11px] text-[#8e7a68]">حالة التاجر</p><p className="mt-1 text-sm font-bold text-[#4f2e1f]">{applicationLabel(application?.status)}</p></div>{user.adminMembership && <div className="rounded-2xl bg-[#f1f6ff] px-3 py-2 text-right"><p className="text-[11px] text-blue-700">عضوية الإدارة</p><p className="mt-1 text-sm font-bold text-blue-900">{user.adminMembership.role?.name_ar ?? user.adminMembership.role?.code ?? "مدير"} — {user.adminMembership.is_active === false ? "معطّلة" : "نشطة"}</p></div>}</div><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><div className="rounded-2xl border border-[#eee1d0] bg-white/70 p-3"><div className="flex items-center gap-2 text-xs font-bold text-[#4f2e1f]"><Clock3 className="size-4 text-[#9c5a00]" />إنشاء الهوية</div><p className="mt-2 text-xs text-[#806b5a]" dir="ltr">{formatDate(user.authCreatedAt ?? user.created_at)}</p></div><div className="rounded-2xl border border-[#eee1d0] bg-white/70 p-3"><div className="flex items-center gap-2 text-xs font-bold text-[#4f2e1f]"><Activity className="size-4 text-[#9c5a00]" />آخر دخول</div><p className="mt-2 text-xs text-[#806b5a]" dir="ltr">{formatDate(user.lastSignInAt)}</p></div><div className="rounded-2xl border border-[#eee1d0] bg-white/70 p-3"><div className="flex items-center gap-2 text-xs font-bold text-[#4f2e1f]"><RefreshCw className="size-4 text-[#9c5a00]" />آخر نشاط مسجل</div><p className="mt-2 text-xs text-[#806b5a]" dir="ltr">{formatDate(user.last_seen_at ?? user.lastActiveAt)}</p></div><div className="rounded-2xl border border-[#eee1d0] bg-white/70 p-3"><div className="flex items-center gap-2 text-xs font-bold text-[#4f2e1f]"><ShieldCheck className="size-4 text-[#9c5a00]" />تأكيد البريد</div><p className="mt-2 text-xs text-[#806b5a]" dir="ltr">{formatDate(user.emailConfirmedAt)}</p></div></div><div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4"><div className="rounded-2xl bg-[#faf6f0] p-3"><p className="text-[11px] font-bold text-[#4f2e1f]">الهاتف</p><p className="mt-2 text-xs text-[#806b5a]" dir="ltr">{user.phone ?? user.profile?.phone ?? "غير مسجل"}</p></div><div className="rounded-2xl bg-[#faf6f0] p-3"><div className="flex items-center gap-2 text-[11px] font-bold text-[#4f2e1f]"><MapPin className="size-3.5 text-[#9c5a00]" />الموقع</div><p className="mt-2 text-xs text-[#806b5a]">{application?.location ?? "غير مسجل"}</p></div><div className="rounded-2xl bg-[#faf6f0] p-3"><div className="flex items-center gap-2 text-[11px] font-bold text-[#4f2e1f]"><Store className="size-3.5 text-[#9c5a00]" />المتجر</div><p className="mt-2 text-xs text-[#806b5a]">{store?.name_ar ?? "لا يوجد متجر"}{store ? ` — ${store.status === "active" ? "نشط" : store.status ?? "غير محدد"}` : ""}</p></div><div className="rounded-2xl bg-[#faf6f0] p-3"><div className="flex items-center gap-2 text-[11px] font-bold text-[#4f2e1f]"><Globe2 className="size-3.5 text-[#9c5a00]" />IP والشبكة</div><p className="mt-2 text-xs text-[#806b5a]">{user.networkTelemetry?.ipAddress ?? "غير مسجل"}</p></div></div>{application?.review_note && <div className="mt-3 rounded-2xl border border-blue-100 bg-blue-50/60 p-3 text-xs leading-6 text-blue-900"><span className="font-bold">ملاحظة مراجعة التاجر:</span> {application.review_note}</div>}<div className="mt-3 flex flex-wrap gap-3 text-[11px] text-[#9a8571]"><span>إرسال الطلب: <b dir="ltr">{formatDate(application?.submitted_at)}</b></span><span>مراجعة الطلب: <b dir="ltr">{formatDate(application?.reviewed_at)}</b></span><span>{user.networkTelemetry?.noteAr ?? "بيانات الشبكة حسب ما هو مسجل فعليًا."}</span></div><div className="mt-4 flex justify-end"><Button onClick={() => void removeUser(user)} disabled={removingUserId === user.id} variant="outline" className="border-red-200 text-red-800"><Trash2 className="ml-2 size-4" />{removingUserId === user.id ? "جارٍ الحذف…" : "حذف المستخدم"}</Button></div></article>; })}</div>}{total > pageSize && <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#eee1d0] px-5 py-4 text-sm text-[#806b5a]"><span>عرض {((page - 1) * pageSize) + 1}–{Math.min(page * pageSize, total)} من أصل {total} مستخدم</span><div className="flex items-center gap-2"><Button disabled={page <= 1 || loading} onClick={() => void load(page - 1, appliedSearch)} variant="outline" className="border-[#eadcc9] text-[#6f4b2d]"><ChevronRight className="ml-1 size-4" />السابق</Button><span className="rounded-lg bg-[#fff7e9] px-3 py-2 font-semibold text-[#4f2e1f]">صفحة {page}</span><Button disabled={page * pageSize >= total || loading} onClick={() => void load(page + 1, appliedSearch)} variant="outline" className="border-[#eadcc9] text-[#6f4b2d]">التالي<ChevronLeft className="mr-1 size-4" /></Button></div></div>}</section>;
}
...
export function NotificationsPanel() {
  ...
  const load = async () => { setLoading(true); setError(null); try { setItems((await adminApi.notifications()).items as NotificationRow[]); } catch (requestError) { const message = requestError instanceof Error ? requestError.message : "تعذر قراءة الإشعارات."; setError(message); toast.error(message); } finally { setLoading(false); } };
  useEffect(() => { void load(); void adminApi.users().then((result) => setUsers(result.items as UserRow[])).catch(() => undefined); }, []);
  const uploadImage = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file) return;
    if (!["image/png", "image/jpeg", "image/webp"].includes(file.type)) { toast.error("اختر صورة PNG أو JPEG أو WebP."); return; }
...
export function AnalyticsPanel() {
  ...
  useEffect(() => { void load(); }, []);
  return <section className="space-y-6"><div className="flex items-center justify-between"><div><div className="section-kicker">التحليل التشغيلي</div><h2 className="mt-1 text-2xl font-bold text-[#342118]">الإحصاءات التشغيلية</h2><p className="mt-2 text-sm leading-7 text-[#806b5a]">مؤشرات مشتقة من جداول Production الحالية فقط؛ لا توجد أرقام تقديرية.</p></div><Button disabled={loading} onClick={() => void load()} variant="outline" className="border-[#eadcc9] text-[#8b5a2b]"><RefreshCw className="ml-2 size-4" />تحديث</Button></div>{loading ? <div className="admin-card p-10 text-center"><RefreshCw className="mx-auto size-5 animate-spin text-[#c77d1a]" /></div> : error ? <div className="admin-card flex flex-wrap items-center justify-between gap-3 p-6 text-sm text-red-800"><span>{error}</span><Button onClick={() => void load()} variant="outline" className="border-red-200 text-red-800"><RefreshCw className="ml-2 size-4" />إعادة المحاولة</Button></div> : !data ? <div className="admin-card flex flex-wrap items-center justify-between gap-3 p-6 text-sm text-[#806b5a]"><span>لم تصل قراءة صالحة للإحصاءات بعد.</span><Button onClick={() => void load()} variant="outline" className="border-[#eadcc9] text-[#8b5a2b]"><RefreshCw className="ml-2 size-4" />إعادة المحاولة</Button></div> : <><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{Object.entries(data?.operational ?? {}).map(([key, value]) => <article key={key} className="admin-card p-5"><div className="grid size-11 place-items-center rounded-2xl bg-[#fff0d6] text-[#9c5a00]"><BarChart3 className="size-5" /></div><p className="mt-5 text-3xl font-bold text-[#342118]">{value}</p><p className="mt-1 text-sm font-semibold text-[#4f2e1f]">{operationalLabels[key] ?? key}</p></article>)}</div><div className="admin-card p-6 text-sm text-[#806b5a]">آخر قراءة: <span dir="ltr">{data?.generatedAt}</span></div></>}</section>;
}
