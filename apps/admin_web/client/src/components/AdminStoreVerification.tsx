...
import { AdminSafeImage, AdminPremiumBadge } from "@/components/AdminVisuals";
...
export function AdminStoreVerificationPanel() {
  ...
  const runAction = async (action: "approve" | "reject" | "needs_more_info" | "revoke") => {
    ...
    try {
      ...
      toast.success(action === "approve" ? "تم اعتماد توثيق المتجر وإرسال الإشعار." : action === "revoke" ? "تم سحب شارة التوثيق وإرسال الإشعار." : action === "needs_more_info" ? "تم طلب استكمال بيانات التوثيق وإرسال الإشعار." : "تم حفظ رفض طلب التوثيق وإرسال الإشعار.");
      setSelected(null);
      setSelectedDetail(null);
      await load();
    } catch (error) {
      const message = error instanceof Error ? error.message : "تعذر تنفيذ قرار التوثيق.";
      toast.error(message);
  ...
  const reconcilePayment = async (paymentStatus: "paid" | "waived" | "failed" | "refunded") => {
    ...
    try {
      ...
      toast.success(labels[paymentStatus] + " بنجاح وتحديث حالة الطلب.");
      await load();
    } catch (error) {
      const message = error instanceof Error ? error.message : "تعذر تسوية رسوم التوثيق.";
      toast.error(message);
      setState((previous) => ({ ...previous, error: message }));
    } finally {
      setBusy(false);
