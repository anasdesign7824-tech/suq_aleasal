...
export function MerchantApplicationsPanel({ state, onRefresh, onReviewed }: { state: LoadState<Application[]>; onRefresh: () => void; onReviewed: () => void }) {
  const review = async (application: Application, status: "approved" | "rejected" | "needs_more_info", reviewNote?: string) => {
    try {
      await adminApi.reviewMerchantApplication(application.id, { status, reviewNote: reviewNote?.trim() || null });
      toast.success("تم حفظ قرار طلب التاجر في Production.");
      onReviewed();
    } catch (error) {
  ...
  return <section className="admin-card overflow-hidden"><div className="flex items-start justify-between gap-4 border-b border-[#eee1d0] px-5 py-5"><div><div className="section-kicker">دورة التاجر</div><h2 className="mt-1 text-xl font-bold text-[#342118]">طلبات التجار</h2><p className="mt-1 text-sm leading-6 text-[#806b5a]">استقبال الطلب من التطبيق ثم اعتماد القرار وتسجيله في سجل التدقيق.</p></div><Button onClick={onRefresh} variant="ghost" size="icon" className="text-[#8b5a2b]"><RefreshCw className="size-4" /></Button></div><PanelState state={state} onRefresh={onRefresh} />{!state.loading && !state.error && (state.data ?? []).length > 0 && <div className="divide-y divide-[#f1e7da]">{(state.data ?? []).map((application) => <article className="p-5" key={application.id}><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-base font-bold text-[#432a1e]">{application.display_name}</h3><p className="mt-1 text-xs text-[#8e7a68]" dir="ltr">{application.phone ?? application.user_id}</p></div>{statusBadge(application.status)}</div><div className="mt-4 grid gap-3 text-sm text-[#6f5b4c] sm:grid-cols-2"><p><span className="font-semibold text-[#4f2e1f]">الموقع:</span> {application.location ?? "غير محدد"}</p><p><span className="font-semibold text-[#4f2e1f]">الخبرة:</span> {application.experience ?? "غير محددة"}</p></div>{formatSpecialties(application.specialties) && <p className="mt-3 text-sm leading-6 text-[#6f5b4c]"><span className="font-semibold text-[#4f2e1f]">التخصصات:</span> {formatSpecialties(application.specialties)}</p>}{application.certificate_note && <p className="mt-2 text-sm leading-6 text-[#6f5b4c]"><span className="font-semibold text-[#4f2e1f]">الشهادة:</span> {application.certificate_note}</p>}<div className="mt-4 flex flex-col gap-2 sm:flex-row"><Input aria-label="ملاحظة قرار التاجر" placeholder="ملاحظة القرار (اختيارية)" defaultValue={application.review_note ?? ""} id={`merchant-note-${application.id}`} className="h-10 rounded-xl border-[#eadcc9]" /><Button onClick={() => void review(application, "approved", (document.getElementById(`merchant-note-${application.id}`) as HTMLInputElement | null)?.value)} className="bg-[#4f2e1f] hover:bg-[#6b412a]"><CheckCircle2 className="ml-2 size-4" />اعتماد</Button><Button onClick={() => void review(application, "needs_more_info", (document.getElementById(`merchant-note-${application.id}`) as HTMLInputElement | null)?.value)} variant="outline" className="border-blue-200 text-blue-800"><Info className="ml-2 size-4" />طلب معلومات</Button><Button onClick={() => void review(application, "rejected", (document.getElementById(`merchant-note-${application.id}`) as HTMLInputElement | null)?.value)} variant="outline" className="border-red-200 text-red-800"><XCircle className="ml-2 size-4" />رفض</Button></div></article>)}</div>}</section>;
}
