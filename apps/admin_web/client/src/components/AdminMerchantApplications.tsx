...
import { adminStatusMeta } from "@/lib/ui-consistency";
...
const statusMeta = (status: string) => adminStatusMeta(status, "merchantApplication");
...
export function MerchantApplicationsPanel({ state, onRefresh, onReviewed }: { state: LoadState<Application[]>; onRefresh: () => void; onReviewed: () => void }) {
  ...
  const review = async (application: Application, status: "approved" | "rejected" | "needs_more_info") => {
    if (reviewingId === application.id) return;
    const confirmation = status === "approved" ? "تفعيل المتجر سيحدّث حساب التاجر ويرسل إشعارًا. هل تريد المتابعة؟" : status === "rejected" ? "رفض طلب التاجر سيمنع تفعيل المتجر. هل تريد المتابعة؟" : "سيُطلب من التاجر استكمال معلومات إضافية. هل تريد المتابعة؟";
