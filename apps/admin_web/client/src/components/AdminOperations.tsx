...
import { Check, ImagePlus, Layers3, MapPin, Pencil, Plus, RefreshCw, Save, Tag, Trash2, Truck } from "lucide-react";
...
type Banner = { id: string; title_ar: string; body_ar?: string | null; image_url?: string | null; cta_label_ar?: string | null; cta_url?: string | null; starts_at?: string | null; ends_at?: string | null; is_active: boolean; sort_order: number };
...
export function BannersPanel() {
  ...
  const [form, setForm] = useState({ titleAr: "", bodyAr: "", imageUrl: "", ctaLabelAr: "استكشف", ctaUrl: "", startsAt: "", endsAt: "", sortOrder: "0" });
  ...
  const create = async (event: FormEvent) => {
    ...
    if (!form.imageUrl) { toast.error("ارفع صورة البانر قبل الحفظ."); return; }
    if (form.startsAt && form.endsAt && new Date(form.endsAt).getTime() <= new Date(form.startsAt).getTime()) { toast.error("يجب أن يكون انتهاء البانر بعد بدايته."); return; }
    setSaving(true);
    try { await adminApi.createBanner({ ...form, startsAt: form.startsAt ? new Date(form.startsAt).toISOString() : null, endsAt: form.endsAt ? new Date(form.endsAt).toISOString() : null, sortOrder: Number(form.sortOrder), isActive: false }); setForm({ titleAr: "", bodyAr: "", imageUrl: "", ctaLabelAr: "استكشف", ctaUrl: "", startsAt: "", endsAt: "", sortOrder: "0" }); toast.success("تم حفظ البانر كمسودة."); await load(); } catch (error) { toast.error(error instanceof Error ? error.message : "تعذر حفظ البانر."); } finally { setSaving(false); }
  };
  const publish = async (banner: Banner) => {
    if (bannerBusyId === banner.id) return;
    const action = banner.is_active ? "إيقاف نشر" : "نشر";
    if (!window.confirm(`هل تريد ${action} البانر «${banner.title_ar}»؟`)) return;
    setBannerBusyId(banner.id);
    try { await adminApi.updateBanner(banner.id, { isActive: !banner.is_active }); toast.success(banner.is_active ? "تم إيقاف البانر." : "تم نشر البانر."); await load(); } catch (error) { toast.error(error instanceof Error ? error.message : "تعذر تحديث حالة البانر."); } finally { setBannerBusyId(null); }
  };
  const uploadImage = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    event.target.value = "";
    if (!file) return;
    if (!("image/png" === file.type || "image/jpeg" === file.type || "image/webp" === file.type)) { toast.error("يسمح برفع PNG أو JPEG أو WebP فقط."); return; }
  ...
  const scheduleLabel = (banner: Banner) => { const now = Date.now(); if (!banner.is_active) return "مسودة/متوقف"; if (banner.starts_at && new Date(banner.starts_at).getTime() > now) return "مجدول"; if (banner.ends_at && new Date(banner.ends_at).getTime() <= now) return "منتهٍ"; return "منشور"; };
...
export function TaxonomyPanel() {
  ...
  useEffect(() => { void load(); }, []);
  const saveCategory = async (event: FormEvent) => { event.preventDefault(); if (savingCategory) return; setSavingCategory(true); try { await adminApi.upsertCategory({ ...categoryForm, id: editingCategoryId ?? undefined }); setCategoryForm({ nameAr: "", slug: "", categoryKind: "honey" }); setEditingCategoryId(null); toast.success("تم حفظ التصنيف."); await load(); } catch (error) { toast.error(error instanceof Error ? error.message : "تعذر حفظ التصنيف."); } finally { setSavingCategory(false); } };
  const saveTaxonomy = async (event: FormEvent) => { event.preventDefault(); if (savingTaxonomy) return; setSavingTaxonomy(true); try { await adminApi.upsertTaxonomy({ ...taxonomyForm, id: editingTaxonomyId ?? undefined }); setTaxonomyForm({ nameAr: "", code: "" }); setEditingTaxonomyId(null); toast.success("تم حفظ تصنيف العسل."); await load(); } catch (error) { toast.error(error instanceof Error ? error.message : "تعذر حفظ تصنيف العسل."); } finally { setSavingTaxonomy(false); } };
  return <section className="space-y-6"><div className="flex items-center justify-between"><div><div className="section-kicker">المرجع القانوني للبيانات</div><h2 className="mt-1 text-2xl font-bold text-[#342118]">التصنيفات والأنواع</h2><p className="mt-2 text-sm leading-7 text-[#806b5a]">مصدر واحد للتصنيفات الهرمية التي يستهلكها التطبيق والإدارة.</p></div><Button onClick={() => void load()} variant="outline" className="border-[#eadcc9] text-[#8b5a2b]"><RefreshCw className="ml-2 size-4" />تحديث</Button></div>{loading ? <div className="admin-card"><PanelState loading error={null} empty={false} onRefresh={() => void load()} /></div> : error ? <div className="admin-card p-6"><PanelState loading={false} error={error} empty={false} onRefresh={() => void load()} /></div> : <><div className="grid gap-6 xl:grid-cols-2"><div className="admin-card p-6"><div className="flex items-center gap-2"><Layers3 className="size-5 text-[#9c5a00]" /><h3 className="font-bold text-[#4f2e1f]">تصنيف العسل المرجعي</h3></div><div className="mt-5 space-y-2">{taxonomies.length ? taxonomies.map((item) => <div key={item.id} className="flex items-center justify-between rounded-xl bg-[#fffaf3] px-3 py-2 text-sm"><span className="font-semibold text-[#432a1e]">{item.name_ar}</span><div className="flex items-center gap-2"><code className="text-xs text-[#8e7a68]">{item.code}</code><Button type="button" onClick={() => { setEditingTaxonomyId(item.id); setTaxonomyForm({ nameAr: item.name_ar, code: item.code }); }} variant="ghost" size="icon" className="size-8 text-[#8b5a2b]" aria-label={`تعديل ${item.name_ar}`}><Pencil className="size-3.5" /></Button></div></div>) : <p className="text-sm text-[#806b5a]">لا توجد أنواع نشطة.</p>}</div><form onSubmit={saveTaxonomy} className="mt-5 grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><Input required value={taxonomyForm.nameAr} onChange={(event) => setTaxonomyForm({ ...taxonomyForm, nameAr: event.target.value })} placeholder="اسم النوع" className="rounded-xl border-[#eadcc9]" /><Input required dir="ltr" value={taxonomyForm.code} onChange={(event) => setTaxonomyForm({ ...taxonomyForm, code: event.target.value })} placeholder="sidr" className="rounded-xl border-[#eadcc9]" /><Button type="submit" disabled={savingTaxonomy} size="icon" className="bg-[#4f2e1f] hover:bg-[#6b412a]"><Plus className="size-4" /></Button></form></div><div className="admin-card p-6"><div className="flex items-center gap-2"><Tag className="size-5 text-[#9c5a00]" /><h3 className="font-bold text-[#4f2e1f]">التصنيفات الهرمية</h3></div><div className="mt-5 space-y-2">{categories.length ? categories.map((item) => <div key={item.id} className="flex items-center justify-between rounded-xl bg-[#fffaf3] px-3 py-2 text-sm"><span className="font-semibold text-[#432a1e]">{item.name_ar}</span><div className="flex items-center gap-2"><span className="text-xs text-[#8e7a68]">{item.slug}</span><Button type="button" onClick={() => { setEditingCategoryId(item.id); setCategoryForm({ nameAr: item.name_ar, slug: item.slug, categoryKind: item.category_kind }); }} variant="ghost" size="icon" className="size-8 text-[#8b5a2b]" aria-label={`تعديل ${item.name_ar}`}><Pencil className="size-3.5" /></Button></div></div>) : <p className="text-sm text-[#806b5a]">لا توجد تصنيفات نشطة.</p>}</div><form onSubmit={saveCategory} className="mt-5 grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><Input required value={categoryForm.nameAr} onChange={(event) => setCategoryForm({ ...categoryForm, nameAr: event.target.value })} placeholder="اسم التصنيف" className="rounded-xl border-[#eadcc9]" /><Input required dir="ltr" value={categoryForm.slug} onChange={(event) => setCategoryForm({ ...categoryForm, slug: event.target.value })} placeholder="honey" className="rounded-xl border-[#eadcc9]" /><Button type="submit" disabled={savingCategory} size="icon" className="bg-[#4f2e1f] hover:bg-[#6b412a]"><Plus className="size-4" /></Button></form></div></div></>}</section>;
}
