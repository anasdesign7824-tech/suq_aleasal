...
import { adminAuditActionLabel, adminAuditEntityLabel } from "@/lib/ui-consistency";
...
export function AdminUsersPanel() {
  ...
  useEffect(() => { void load(); }, []);
  const toggle = async (item: AdminUser) => { if (updatingId === item.user_id) return; const action = item.is_active ? "تعطيل" : "تفعيل"; if (!window.confirm(`هل تريد ${action} عضوية «${item.name ?? item.email ?? item.user_id}»؟`)) return; setUpdatingId(item.user_id); try { await adminApi.updateAdminUser(item.user_id, { isActive: !item.is_active }); toast.success(item.is_active ? "تم تعطيل العضوية." : "تم تفعيل العضوية."); await load(); } catch (requestError) { toast.error(requestError instanceof Error ? requestError.message : "تعذر تحديث العضوية."); } finally { setUpdatingId(null); } };
  return <section className="admin-card overflow-hidden"><div className="flex items-start justify-between border-b border-[#eee1d0] px-5 py-5"><div><div className="section-kicker">الهوية والصلاحيات</div><h2 className="mt-1 text-xl font-bold text-[#342118]">المديرون</h2><p className="mt-2 text-sm leading-7 text-[#806b5a]">هوية الإدارة مستقلة عن Customer Auth وOTP، وعضويتها مربوطة بالدور والصلاحيات.</p></div><Button onClick={() => void load()} variant="ghost" size="icon" className="text-[#8b5a2b]"><RefreshCw className="size-4" /></Button></div>{loading ? <div className="p-8 text-center text-sm text-[#806b5a]"><RefreshCw className="mx-auto size-5 animate-spin text-[#c77d1a]" /></div> : error ? <div className="flex flex-wrap items-center justify-between gap-3 p-6 text-sm text-red-800"><span>{error}</span><Button onClick={() => void load()} variant="outline" className="border-red-200 text-red-800"><RefreshCw className="ml-2 size-4" />إعادة المحاولة</Button></div> : <div className="divide-y divide-[#f1e7da]">{items.map((item) => <div key={item.user_id} className="flex flex-wrap items-center gap-3 px-5 py-4"><div className="grid size-10 place-items-center rounded-2xl bg-[#f3e9df] text-[#4f2e1f]"><UserCog className="size-5" /></div><div className="min-w-48 flex-1"><p className="font-semibold text-[#432a1e]">{item.name ?? "مدير عسلكم"}</p><p className="mt-1 text-xs text-[#806b5a]" dir="ltr">{item.email ?? "لا يوجد بريد"}</p></div><Badge variant="outline" className="border-[#e3c28d] text-[#8b5a2b]">{item.role?.name_ar ?? "دون اسم دور"}</Badge><Badge variant="outline" className={item.is_active ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-red-300 bg-red-50 text-red-800"}>{item.is_active ? "نشط" : "معطل"}</Badge><Button disabled={updatingId === item.user_id} onClick={() => void toggle(item)} variant="outline" size="sm" className="border-[#eadcc9] text-[#8b5a2b]">{item.is_active ? "تعطيل" : "تفعيل"}</Button></div>)}{items.length === 0 && <div className="p-8 text-center text-sm text-[#806b5a]">لا توجد عضويات إدارة.</div>}</div>}</section>;
}
