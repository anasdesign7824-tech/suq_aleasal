...
export function AdminCreationPanel() {
  ...
  const create = async () => {
    ...
    try {
      ...
      setEmail("");
      toast.success("تم إنشاء Admin Identity وربطها بالدور.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "تعذر إنشاء المدير.");
    } finally { setSaving(false); }
  };
  return <section className="admin-card p-6"><div className="flex items-start gap-3"><div className="grid size-11 place-items-center rounded-2xl bg-[#fff0d6] text-[#9c5a00]"><UserPlus className="size-5" /></div><div><div className="section-kicker">Admin Identity</div><h2 className="mt-1 text-xl font-bold text-[#342118]">إنشاء مدير فرعي</h2><p className="mt-2 text-sm leading-7 text-[#806b5a]">ينفذها Super Admin فقط. كلمة المرور لا تُخزن في قاعدة البيانات وتظهر مرة واحدة.</p></div></div><div className="mt-5 grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_auto]"><Input dir="ltr" type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="admin@example.com" className="rounded-xl border-[#eadcc9]" /><Input dir="ltr" required value={roleCode} onChange={(event) => setRoleCode(event.target.value)} placeholder="moderator" className="rounded-xl border-[#eadcc9]" /><Button disabled={saving || !email.trim()} onClick={() => void create()} className="bg-[#4f2e1f] hover:bg-[#6b412a]"><UserPlus className="ml-2 size-4" />إنشاء</Button></div>{temporaryPassword && <div className="mt-5 rounded-2xl border border-amber-300 bg-amber-50 p-4"><div className="flex items-center gap-2 text-sm font-bold text-amber-900"><KeyRound className="size-4" />بيانات التسليم لمرة واحدة</div><p className="mt-2 text-xs leading-6 text-amber-900">انسخ كلمة المرور وسلّمها للمدير خارج الواجهة، ثم اطلب تغييرها بعد الدخول الأول.</p><div className="mt-3 grid gap-2 rounded-xl bg-white/70 p-3 text-sm" dir="ltr"><code className="select-all font-semibold text-[#4f2e1f]">{temporaryPassword}</code><Button variant="outline" size="sm" onClick={() => { void navigator.clipboard?.writeText(temporaryPassword); toast.success("تم نسخ كلمة المرور."); }} className="border-amber-300 text-amber-900">نسخ</Button></div></div>}</section>;
}
