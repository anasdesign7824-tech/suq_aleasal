...
type Campaign = { id: string; name_ar: string; discount_percent: number; is_active: boolean; starts_at: string | null; ends_at: string | null; applies_to: string[] };
...
export function AdminPlansPanel() {
  ...
  const refresh = async () => {
    setLoading(true);
    try {
      const [planResult, campaignResult, paymentResult, subscriptionResult, designResult, transferResult] = await Promise.all([
        adminApi.subscriptionPlans(), adminApi.subscriptionCampaigns(), adminApi.paymentRequests({ page: 1, pageSize: 50 }), adminApi.subscriptions({ page: 1, pageSize: 50 }), adminApi.designRequests({ page: 1, pageSize: 50 }), adminApi.localTransferSettings(),
      ]);
      ...
      setDiscount(String(activeCampaign?.discount_percent ?? 15));
  ...
  const monthlyPlans = useMemo(() => plans.filter((plan) => plan.billing_interval === "month"), [plans]);
  const saveCampaign = async () => {
    try { await adminApi.updateLaunchCampaign({ discountPercent: Number(discount), isActive: campaign?.is_active ?? false, appliesTo: campaign?.applies_to ?? ["subscription", "verification"] }); toast.success("تم حفظ حملة الافتتاح."); await refresh(); } catch (error) { toast.error(error instanceof Error ? error.message : "تعذر حفظ الحملة."); }
  };
  const toggleCampaign = async () => {
    try { await adminApi.updateLaunchCampaign({ discountPercent: Number(discount), isActive: !(campaign?.is_active ?? false), appliesTo: campaign?.applies_to ?? ["subscription", "verification"] }); toast.success(campaign?.is_active ? "تم إيقاف الخصم للطلبات الجديدة." : "تم تفعيل خصم الافتتاح."); await refresh(); } catch (error) { toast.error(error instanceof Error ? error.message : "تعذر تعديل الحملة."); }
  };
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">{monthlyPlans.map((plan) => <PlanCard key={plan.id} plan={plan} launchDiscount={campaign?.is_active ? Number(discount) : 0} onRefresh={() => void refresh()} />)}</section>
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,.75fr)]"><div className="admin-card p-6"><div className="flex items-start justify-between gap-3"><div><p className="section-kicker">إدارة الخصم</p><h3 className="mt-1 text-xl font-bold text-[#342118]">حملة افتتاح التطبيق</h3></div><Badge variant="outline" className={campaign?.is_active ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-slate-300 bg-slate-50 text-slate-700"}>{campaign?.is_active ? "نشطة" : "متوقفة"}</Badge></div><div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end"><label className="flex-1 text-sm font-semibold text-[#5f4636]">نسبة الخصم الحالية<input value={discount} onChange={(event) => setDiscount(event.target.value)} type="number" min="0" max="100" className="mt-2 h-11 w-full rounded-xl border border-[#eadcc9] bg-white px-3" /></label><Button onClick={() => void saveCampaign()} className="bg-[#4f2e1f] hover:bg-[#6b412a]">حفظ النسبة</Button><Button onClick={() => void toggleCampaign()} variant="outline" className="border-[#cda36d] text-[#76502e]">{campaign?.is_active ? "إيقاف الخصم" : "تفعيل الخصم"}</Button></div><p className="mt-4 text-xs leading-6 text-[#806b5a]">لا تتغير الطلبات التي دُفعت أو فُعّلت عند إيقاف الحملة. النطاق الحالي: اشتراكات الخطط والتوثيق الأولي.</p></div><div className="admin-card p-6"><div className="flex items-center gap-2 text-[#9c5a00]"><WalletCards className="size-5" /><h3 className="text-lg font-bold text-[#342118]">الدفع المتاح</h3></div><p className="mt-3 text-sm leading-7 text-[#806b5a]">الحوالة المحلية فقط. خيار البطاقة معطل ولا توجد حقول بطاقة أو بيانات CVV في هذا المسار.</p><div className="mt-4 rounded-2xl bg-[#fff7e9] p-4 text-sm text-[#6f4b2d]">{transfer.is_active ? `الحساب النشط: ${transfer.bank_name ?? "غير محدد"}` : "لم تُفعّل بيانات الحوالة بعد."}</div></div></section>
