...
function numericQuery(value: unknown, fallback: number): number {
  ...
  const max = fallback === 20 ? 100 : 100_000;
...
async function startServer() {
  ...
  app.get("/api/admin/users", requireAdmin("user.read"), async (request, response) => {
    try {
  ...
  app.patch("/api/admin/store-verification-requests/:id", requireAdmin("verification.review"), async (request, response) => {
    try {
      const id = Array.isArray(request.params.id) ? request.params.id[0] : request.params.id;
