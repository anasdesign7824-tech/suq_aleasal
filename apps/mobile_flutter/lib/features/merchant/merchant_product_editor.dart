...
class _MerchantProductEditorScreenState
  ...
  Future<void> _save() async {
    ...
    if (result is AssalData<AssalProductSummary>) {
      final productId = result.value.id;
      String? imageError;
      ...
      if (imageError != null) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('تم حفظ المنتج، لكن تعذر رفع صورة: $imageError')),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(widget.product == null
                ? 'تم حفظ المنتج كمعلّق. يمكنك معاينته الآن.'
  ...
  Widget _qualityTab() => ListView(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(child: _field(weightController, 'الوزن أو الحجم')),
              const SizedBox(width: AssalSpacing.sm),
              Expanded(
          ...
          const SizedBox(height: AssalSpacing.md),
          _field(originController, 'بلد المصدر'),
          const SizedBox(height: AssalSpacing.md),
