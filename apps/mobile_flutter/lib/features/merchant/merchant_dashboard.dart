...
class _MerchantDashboardState extends State<MerchantDashboard> {
  ...
  Future<AssalLoadState<AssalMerchantWorkspaceSummary?>>
    ...
    if (session.isUnavailable) {
      return AssalError(
        session.errorMessageAr ?? 'تعذر مزامنة حساب التاجر الآن.',
  ...
  Future<void> _deleteProduct(AssalProductSummary product) async {
    ...
    if (!session.isAuthenticated || session.user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('سجّل الدخول قبل حذف المنتج.')),
      );
      return;
    }
...
class _MerchantStoreEditorScreenState extends State<MerchantStoreEditorScreen> {
  ...
  Future<void> _pickBrandImage({required bool cover}) async {
    ...
    if (!session.isAuthenticated || session.user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('سجّل الدخول قبل رفع صور المتجر.')),
      );
      return;
    }
  ...
  Future<void> _pickGalleryImage() async {
    ...
    if (!session.isAuthenticated || session.user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('سجّل الدخول قبل رفع صور المعرض.')),
      );
      return;
    }
    final picked = await ImagePicker().pickImage(
  ...
  Future<void> _save() async {
    ...
    if (!session.isAuthenticated || session.user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('سجّل الدخول قبل حفظ بيانات المتجر.')),
      );
      return;
    }
