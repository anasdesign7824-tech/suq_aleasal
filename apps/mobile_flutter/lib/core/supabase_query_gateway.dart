...
class SupabaseQueryGateway implements ProductionQueryGateway {
  ...
  Future<List<Map<String, Object?>>> select(
    ...
    developer.log(
        'select_start table=$table filter_keys=${filters.keys.join(',')}',
  ...
  Future<Map<String, Object?>> insert(
    ...
    final started = DateTime.now();
    developer.log(
        'insert_start table=$table value_keys=${values.keys.join(',')}',
        name: 'assalkom.network');
    try {
  ...
  Future<Map<String, Object?>> update(String table, Map<String, Object?> values,
    ...
    final started = DateTime.now();
    developer.log(
        'update_start table=$table value_keys=${values.keys.join(',')}',
        name: 'assalkom.network');
    try {
      final row = await client
          .from(table)
  ...
  Future<void> delete(
    ...
    final started = DateTime.now();
    developer.log(
        'delete_start table=$table filter_keys=${filters.keys.join(',')}',
        name: 'assalkom.network');
    try {
  ...
  Future<Map<String, Object?>> rpc(
    ...
    try {
      final value =
          await client.rpc(function, params: params).timeout(requestTimeout);
      if (value is! Map) {
  ...
  Future<Map<String, Object?>> upsert(
    ...
    final started = DateTime.now();
    developer.log(
        'upsert_start table=$table value_keys=${values.keys.join(',')}',
        name: 'assalkom.network');
    try {
      final row = await client
          .from(table)
