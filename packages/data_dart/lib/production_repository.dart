...
class ProductionRepository implements AssalRepository {
  ...
  AssalErrorKind _errorKind(String raw) {
    ...
    if (value.contains('42501') ||
      ...
      return AssalErrorKind.unauthorized;
    }
    if (value.contains('pgrst205') ||
        value.contains('pgrst204') ||
        value.contains('column') ||
        value.contains('relation') ||
        value.contains('schema')) {
      return AssalErrorKind.schemaMismatch;
    }
    if (value.contains('22023') ||
        value.contains('required') ||
        value.contains('invalid') ||
        value.contains('empty')) {
      return AssalErrorKind.validation;
    }
    return AssalErrorKind.server;
  }

  AssalError<T> _failure<T>(
    String message, {
    String? code,
    AssalErrorKind? kind,
    ...
    final resolvedKind = kind ?? _errorKind(source?.toString() ?? code ?? '');
    return AssalError<T>(
      message,
      code: code,
      kind: resolvedKind,
      retryable: retryable || resolvedKind == AssalErrorKind.network,
    );
  }
  ...
  Future<AssalLoadState<T>> _write<T>({
    ...
    try {
      ...
      developer.log(
        'write_timeout resource=$resource',
        name: 'assalkom.production',
      );
      return _failure(
      ...
      final code = isSchema
          ...
          : (isAuth
                ? 'permission_denied'
                : (isNetwork ? 'network' : 'data_write_failed'));
      ...
      developer.log(
        'write_failed resource=$resource code=$code',
        name: 'assalkom.production',
      );
      return _failure(
        message,
        code: code,
        kind: isSchema
            ? AssalErrorKind.schemaMismatch
            : (isAuth
                  ? AssalErrorKind.unauthorized
                  : (isNetwork
                        ? AssalErrorKind.network
                        : AssalErrorKind.server)),
        retryable: isNetwork,
        source: error,
      );
    }
  ...
  Future<AssalLoadState<List<T>>> _readList<T>({
    ...
    try {
      ...
      return _failure<List<T>>(
        ...
        kind: isSchema
            ? AssalErrorKind.schemaMismatch
            : (isNetwork ? AssalErrorKind.network : AssalErrorKind.server),
        retryable: isNetwork,
        source: error,
      );
    }
  ...
  Future<AssalSession> getSession() async {
    ...
    try {
      ...
      developer.log(
        'session_hydration_failed identity=${identity.id}',
        name: 'assalkom.production',
      );
      _clearSessionCache();
      return AssalSession.unavailable;
  ...
  Future<AssalLoadState<AssalSession>> _authOperation(
    ...
    try {
      ...
      return _failure(
        error.messageAr,
        code: error.code,
        kind: AssalErrorKind.unauthorized,
        source: error,
      );
    } on Object catch (error) {
      return _failure(
        ...
        case AssalSort.newest:
          values = [...values]
            ..sort((a, b) {
              ...
              return byDate != 0 ? byDate : a.nameAr.compareTo(b.nameAr);
            });
  ...
  DateTime _productDate(Map<String, Object?> row) {
    ...
    return parse(row['production_date']) ??
        parse(row['packaged_date']) ??
  ...
  Future<AssalLoadState<List<AssalCommentSummary>>> listComments(
    ...
    read: () async {
      ...
      return rows.map(AssalCommentSummary.fromJson).toList(growable: false);
    },
  );
  ...
  Future<AssalLoadState<bool>> markNotificationRead(
    ...
    write: () async {
      ...
      await _gateway.update('notifications', {
  ...
  Future<AssalLoadState<AssalMerchantWorkspaceSummary?>> loadMerchantWorkspace(
    ...
    write: () async {
      ...
      final storeRow = Map<String, Object?>.from(stores.first);
      if (storeRow['contact_phone'] == null) {
        storeRow['contact_phone'] = storeRow['phone'];
      }
      final storeId = storeRow['id'];
      if (storeId is String) {
        final galleryRows = await _gateway.select(
          'store_gallery',
          filters: {'store_id': storeId},
        );
        galleryRows.sort((left, right) {
          final leftOrder = (left['sort_order'] as num?)?.toInt() ?? 0;
          final rightOrder = (right['sort_order'] as num?)?.toInt() ?? 0;
          return leftOrder.compareTo(rightOrder);
        });
        storeRow['gallery_urls'] = galleryRows
            .map((row) => row['media_url'])
            .whereType<String>()
            .toList(growable: false);
      }
      final store = AssalStoreSummary.fromJson(storeRow);
      final status = '${storeRow['status'] ?? 'pending'}';
      ...
      final hasExpiredBadge = badges.any((row) {
      ...
      final hasRevokedBadge = badges.any((row) => row['status'] == 'revoked');
      ...
      final designRequests = activeSubscription == null
          ...
          : await _gateway.select(
              'design_requests',
              filters: {'subscription_id': activeSubscription['id']},
            );
      verificationRequests.sort(
        (left, right) => '${right['created_at'] ?? ''}'.compareTo(
          '${left['created_at'] ?? ''}',
        ),
      );
      ...
      final latestVerificationStatus =
      ...
      return AssalMerchantWorkspaceSummary(
        ...
        storeLimit: plan == null
            ? 1
            : (plan['store_limit'] as num?)?.toInt() ?? 1,
        productLimit: plan == null
            ? 25
            : (plan['product_limit'] as num?)?.toInt() ?? 25,
        designRequestsRemaining: plan == null
            ? 0
            : ((plan['entitlements'] is Map)
                      ? (((plan['entitlements']
                                        as Map)['design_requests_per_cycle']
                                    as num?)
                                ?.toInt() ??
                            0)
                      : 0) -
  ...
  Future<AssalLoadState<void>> updateMerchantWorkspace(
    ...
    write: () async {
      ...
      if (ownedStores.isEmpty) throw StateError('merchant_workspace_not_owned');
      await _gateway.update('stores', {
  ...
  Future<AssalProductSummary> _productSummaryFromRow(
    ...
    final images = await _gateway.select(
      'product_images',
      filters: {'product_id': row['id']},
    );
  ...
  Future<AssalLoadState<List<AssalProductSummary>>> listMerchantProducts(
    ...
    read: () async {
      ...
      for (final storeId in storeIds) {
        rows.addAll(
          await _gateway.select('products', filters: {'store_id': storeId}),
        );
      }
  ...
  String _verificationPaymentWire(VerificationPaymentStatus value) =>
      switch (value) {
  ...
  String _verificationDocumentWire(VerificationDocumentType value) =>
      switch (value) {
  ...
  @override
  Future<AssalLoadState<AssalStoreVerificationSummary>> submitStoreVerification(
    String userId,
    String requestId,
  ) => _write(
    ...
    write: () async {
      ...
      final updated = await _gateway.update('store_verification_requests', {
        'status': 'submitted',
  ...
  @override
  Future<AssalLoadState<AssalStoreVerificationSummary>> addVerificationDocument(
    String userId,
    String requestId,
  ...
  Future<AssalLoadState<List<AssalSubscriptionPlan>>> listSubscriptionPlans() =>
      _readList(
        ...
        read: () async {
          ...
          final orderedRows = List<Map<String, Object?>>.from(rows)
            ..sort((left, right) {
              final leftOrder = (left['sort_order'] as num?)?.toInt() ?? 0;
              final rightOrder = (right['sort_order'] as num?)?.toInt() ?? 0;
          ...
          return orderedRows
              .map(AssalSubscriptionPlan.fromJson)
              .toList(growable: false);
        },
      );

  @override
  Future<AssalLoadState<AssalSubscriptionCampaign?>>
  loadSubscriptionCampaign() => _write(
  ...
  @override
  Future<AssalLoadState<AssalLocalTransferSettings?>>
  loadLocalTransferSettings() => _write(
    resource: 'local_transfer_settings.read',
    write: () async {
      final rows = await _gateway.select(
        'local_transfer_settings',
        filters: {'code': 'primary', 'is_active': true},
      );
      if (rows.isEmpty) return null;
  ...
  Future<AssalLoadState<AssalPaymentRequest>> submitPaymentProof(
    ...
    write: () async {
      final row = await _gateway.rpc('merchant_submit_payment_proof', {
        ...
        'p_transfer_date': transferDate.toUtc().toIso8601String().substring(
          0,
          10,
        ),
